...
public class SimpleStatement extends RegularStatement {
    ...
    private final Object []objValues;
    ...
    private static ByteBuffer[] convert(Object[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            try {
            	if (values[i] !=null)
            		serializedValues[i] =  TypeCodec.getDataTypeFor(values[i]).serialize(values[i]);  //DataType.serializeValue(values[i]);
            	else
    ...
    /**
     * build the query string substituting "?" with the object values.
     * 
     * NOTE: this is a complete work-around till 2.0 is used.
     * 
     * @return
     */
    public String buildQueryString() {
    	...
    	while (!done) {
    		...
    		int toIndex = query.indexOf("?", beginIndex);
    		
    		if (toIndex != -1) {
    			if (count > objValues.length-1)
    				break;
    			builder.append(query.substring(beginIndex,toIndex));
    			...
    			builder.append(query.substring(beginIndex));
    			done = true;
    		}
    		
    	}
    	//iterate through the object values, substituting 
